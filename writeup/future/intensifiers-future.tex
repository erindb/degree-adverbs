...
\ndg{some redundancy in previous two paragraphs? combine them?}
...
This meta-linguistic hypothesis would appear to be less parsimonious than the pragmatic hypothesis, since the pragmatic hypothesis relies only on mechanisms (M-implicature) that we know to be involved in other examples of language understanding.
...
Motivated by a recent probabilistic model of scalar adjectives \cite{lassiter}, we argued that adjectival intensifiers could get aspects of their meaning through a systematic pragmatic inference, even in the absence of conventional literal meaning. Our model predicted a linear relationship between the intensity of an intensifier and its cost, measured here in terms of length and log-frequency.
...
\ndg{need to expand discussion a lot. draw out the implications of this work for theories of meaning. how could these results extend to other word types? how does this relate to other results on non-arbitrary meanings? perhaps briefly highlight the corpus / experimentalmethodology. discuss the issues with our work so far and the future directions more.}
