...
We asked participants to provide judgments of prices based on a person's description of an object. The description included an intensifier paired with \w{expensive} (Figure~\ref{exp1-q}).
...
For all of the analyses that follow, we use \w{surprisal} or \w{self-information}, defined as the negative log probability of a word in the corpus, as the utterance cost measure. We do this so that our correlations for both utterance cost measures will be positive (if our findings are consistent with our hypothesis) and because \todo{ndg: reasons}. The syllable lenths of our intensifiers and the surprisals %\todo[inline]{say what surprisal is and why we care before using it.} 
